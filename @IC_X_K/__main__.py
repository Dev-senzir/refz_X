...
import IC_X_K
from IC_X_K import BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
try:
    ...
    IC_X_K.loop.run_until_complete(setup_bot())
...
try:
    ...
    IC_X_K.loop.run_until_complete(mybot())
...
async def startup_process():
    if IC_X_K.uid in blacklisted_users:
        LOGS.info("انت لا يمكنك تنصيب سورس ريفز عزيزي دي")
        return
    if not gvarstatus("TNSEEB"):
        try:
            ...
            LOGS.info(
                ...
                \nمجموعة قناة السورس  https://t.me/rev_fxx"
            )
            LOGS.info("============================================================")
            await verifyLoggerGroup()
...
IC_X_K.loop.run_until_complete(startup_process())

if len(sys.argv) in {1, 3, 4}:
    with contextlib.suppress(ConnectionError):
        IC_X_K.run_until_disconnected()
else:
    IC_X_K.disconnect()
