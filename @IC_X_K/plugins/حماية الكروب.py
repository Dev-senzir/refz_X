...
@IC_X_K.ar_cmd(pattern="قفل ?(.*)", groups_only=True, require_admin=True)
async def _(event):
...
@IC_X_K.ar_cmd(pattern="فتح ?(.*)", groups_only=True, require_admin=True)
async def _(event):
    input_str = event.pattern_match.group(1)
    chat_id = event.chat_id
    if not event.is_group:
...
@IC_X_K.ar_cmd(pattern="الاعدادات$", groups_only=True)
async def _(event):
    res = ""
...
@IC_X_K.ar_cmd(incoming=True, forword=None)
async def check_incoming_messages(event):
    ...
    if is_locked(chat_id, "rtl") and (
        ...
        if IC_X_K == razan or await is_admin(event, IC_X_K) or IC_X_K in devs:
            return
        else:
            try:
                await event.delete()
...
@IC_X_K.ar_cmd(pattern=f"البوتات ?(.*)")
