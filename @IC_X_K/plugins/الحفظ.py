...
@
IC_X_K.ar_cmd(pattern="حفظ كتابة$")
async def save(e):
...
@
IC_X_K.ar_cmd(pattern="حفظ توجيه$")
async def saf(e):
    razan = await e.get_reply_message()
    if not razan:
        return await edit_delete(
...
async def roz(event):
    ...
    R7 = await 
IC_X_K.inline_query(BE, razan)
    await R7[0].click(event.chat_id)
    await event.delete()


@
IC_X_K.ar_cmd(pattern="ايجاد الفايروسات$")
async def _(event):
