...
@IC_X_K.ar_cmd(pattern="وقت(?:\s|$)([\s\S]*)")
async def variable(event):
    if Config.HEROKU_API_KEY is None:
        return await edit_delete(
            event,
...
@IC_X_K.ar_cmd(pattern="استخدامي$")
...
@IC_X_K.ar_cmd(pattern="زخرفة الوقت (.*)")
async def variable(var):
    if Config.HEROKU_API_KEY is None:
        return await edit_delete(
            var,
