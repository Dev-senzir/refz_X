...
@IC_X_K.ar_cmd(pattern="اكينوتر(?:\s|$)([\s\S]*)")
async def rozdo(e):
...
@IC_X_K.tgbot.on(CallbackQuery(data=re.compile(b"aki_?(.*)")))
@check_owner
async def daj(e):
...
@IC_X_K.tgbot.on(CallbackQuery(data=re.compile(b"aka_?(.*)")))
@check_owner
async def rooks(e):
