...
from IC_X_K.Config import Config
from IC_X_K.core.managers import edit_or_reply, edit_delete
from IC_X_K.helpers import reply_id
from IC_X_K.sql_helper.globals import gvarstatus
from IC_X_K.plugins import spamwatch
...
async def fetch_info(replied_user, event):
    ...
    rozrtba = (
        ".「  مـطـور آلُِسورس 𓄂𓆃 」."
        if user_id == 5656828413 or user_id == 6643261074 or user_id == 568865 or user_id == 550880 or user_id == 525601 or user_id == 183303
        else (".「  العضـو 𓅫 」.")
    )
    rozrtba = (
        ".「 مـالك الحساب  」."
        if user_id == (await event.client.get_me()).id
        and user_id != 6301863282
        and user_id != 574528
        and user_id != 591695
        and user_id != 525601
        and user_id != 568865
    ...
    caption = " ╮•⎚ مـعلومات الـشخص مـن بـوت ريفز\n"
    caption += f"✛┈┉━｢ 𝗧𝗟𝗘𝗧𝗛𝗢𝗡 𝗥𝗘𝗙𝗭 . 🕷 ｣━┅┈✛\n"
...
@IC_X_K.ar_cmd(pattern="رابط الحساب(?:\s|$)([\s\S]*)")
