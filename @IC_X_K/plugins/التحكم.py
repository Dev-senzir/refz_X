...
@IC_X_K.ar_cmd(pattern="اوامر التحكم")
async def hi(event):
    await edit_or_reply(
        event,
        "⩹⌯⊷━♢ ⦓ 
𝗧𝗟𝗘𝗧𝗛𝗢𝗡 𝗥𝗘𝗙𝗭⦔ ♢━⊶⌯⩺\n\n• اوامر التحكم تتيح لمستخدم اخر ان يستخدم اوامر تنصيبك وهو غير منصب 
ريفز حيث سيصبح متحكم في اوامر تنصيبك يرجى الانتباه من انه سيحصل على صلاحيات الاوامر  وقد يسبب خطورة لك اذا كنت لا تثق في المستخدم الذي اضفته. \n\n\n`.التحكم` تفعيل/تعطيل\n• تستخدم هذه الميزة لتفعيل/لتعطيل خاصية التحكم بأوامر تنصيبك للمستخدمين الذين أضفتهم في قائمة المتحكمين\n\n`.المتحكمين`\n• لعرض المستخدمين الذين اضفتهم الى قائمة المستخدمين المتحكمين\n\n`.اضف متحكم`\n• بالرد على المستخدم لأضافته متحكم في تنصيب 
ريفز الخاص بك\n\n`.ازالة متحكم`\n• بالرد على المستخدم لازالته من قائمة المتحكمين\n⩹⌯⊷━♢ ⦓ 
𝗧𝗟𝗘𝗧𝗛𝗢𝗡 𝗥𝗘𝗙𝗭⦔ ♢━⊶⌯⩺\n**⌯︙ [┈┉━｢ 
𝗧𝗟𝗘𝗧𝗛𝗢𝗡 𝗥𝗘𝗙𝗭🕷 ｣━┅┈](t.me/def_Zoka)**",
        link_preview=False,
    )
...
@IC_X_K.ar_cmd(pattern="التحكم (تفعيل|تعطيل)$")
...
@IC_X_K.ar_cmd(pattern="اضف متحكم(?:\s|$)([\s\S]*)")
...
@IC_X_K.ar_cmd(pattern="ازالة متحكم(?:\s|$)([\s\S]*)")
async def _(event):
    replied_user, error_i_a = await get_user_from_event(event)
    if replied_user is None:
        return
...
@IC_X_K.ar_cmd(pattern="المتحكمين$")
async def _(event):
    ...
    if len(sudochats) == 0:
        return await edit_delete(
            event, "**• يبدو انك لم تقوم بأضافة مستخدم متحكم ل
ريفز الخاص بك**"
        )
