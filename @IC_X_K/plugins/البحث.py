...
from . import IC_X_K, song_download
...
@IC_X_K.ar_cmd(pattern="بحث(320)?(?:\s|$)([\s\S]*)")
...
@IC_X_K.ar_cmd(pattern="فيديو(?:\s|$)([\s\S]*)")
...
@IC_X_K.ar_cmd(pattern="(ا(ل)?ا(س)?م)(?:\s|$)([\s\S]*)")
    ...
    if flag == "s":
        ...
        async with event.client.conversation(chat) as conv:
            try:
                ...
                await IC_X_K(unblock("DeezerMusicBot"))
                purgeflag = await conv.send_message("/start")
