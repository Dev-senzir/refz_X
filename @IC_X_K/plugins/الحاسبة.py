...
@
IC_X_K.ar_cmd(pattern="الحاسبة(?:\s|$)([\s\S]*)")
async def icalc(e):
    if e.client._bot:
        return await e.reply("**الحاسبة العلمية لسورس 
ريفز\n @
def_Zoka**", buttons=lst)
...
async def inlinecalc(event):
    ...
    if (
        ...
        calc = event.builder.article(
            "Calc", text="**الحاسبة العلمية لسورس 
ريفز\n @
def_Zoka**", buttons=lst
        )
...
IC_X_K
@
IC_X_K.tgbot.on(CallbackQuery(data=re.compile(b"calc(.*)")))
@check_owner
...
IC_X_K
@
IC_X_K.tgbot.on(CallbackQuery(data=re.compile(b"recalc")))
@check_owner
async def _(e):
    m = [
        "AC",
    ...
    lst = list(zip(tultd[::4], tultd[1::4], tultd[2::4], tultd[3::4]))
    lst.append([Button.inline("=", data="calc=")])
    await e.edit("**الحاسبة العلمية لسورس 
ريف6301863282ز\n @
def_Zoka**", buttons=lst)
