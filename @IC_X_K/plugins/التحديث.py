...
from IC_X_K import HEROKU_APP, UPSTREAM_REPO_URL, IC_X_K
...
@IC_X_K.ar_cmd(pattern="تحديث(| الان)?$")
async def upstream(event):
    ...
    try:
        txt = "فشل في التحديث لسورس ريفز " + "**• حدث خطأ ما :**\n"
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"**• ارسل** `{cmdhd}تحديث التنصيب` لتحديث سورس ريفز"
        )

    if force_update:
        await event.edit("**• جار التحديث الاجباري الى اخر اصدار انتظر قليلا**")
    if conf == "الان":
        await event.edit("**• جار تحديث سورس ريفز أنتظر قليلا**")
...
async def upstream(event):
    ...
    off_repo = "https://github.com/githupsource/refz_X"
    os.chdir("/app")
    try:
