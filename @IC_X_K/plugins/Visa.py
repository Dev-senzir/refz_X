...
@IC_X_K.ar_cmd(pattern="cc(?:\s|$)([\s\S]*)")
...
@IC_X_K.ar_cmd(pattern="فيزا(?:\s|$)([\s\S]*)")
async def song2(event):
    chat = "@SDBB_Bot"
    reply_id_ = await reply_id(event)
...
@IC_X_K.ar_cmd(pattern="توليد(?:\s|$)([\s\S]*)")
async def song2(event):
...
@IC_X_K.ar_cmd(pattern="ماستر(?:\s|$)([\s\S]*)")
async def song2(event):
...
@IC_X_K.ar_cmd(pattern="اماكس(?:\s|$)([\s\S]*)")
async def song2(event):
    card = event.pattern_match.group(1)
    chat = "@SDBB_Bot"
    reply_id_ = await reply_id(event)
