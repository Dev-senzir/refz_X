...
@
IC_X_K.ar_cmd(pattern="الغاء تقليد$")
async def echo(event):
    if event.reply_to_msg_id is None:
...
@
IC_X_K.ar_cmd(pattern="حذف المقلدهم( للكل)?")
async def echo(event):
...
@
IC_X_K.ar_cmd(pattern="المقلدهم( للكل)?$")
async def echo(event):
    input_str = event.pattern_match.group(1)
    private_chats = ""
...
@
IC_X_K.ar_cmd(incoming=True, edited=False)
