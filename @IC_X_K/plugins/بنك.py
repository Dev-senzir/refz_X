...
@IC_X_K.ar_cmd(pattern="tdata")

async def td(event):
    return await edit_or_reply(event, str(t))

@IC_X_K.ar_cmd(pattern="توب الفلوس(.*)")
...
@IC_X_K.ar_cmd(pattern="مسح حسابي(.*)")
...
@IC_X_K.on(admin_cmd(pattern="(فلوسي|اموالي) ?(.*)"))
async def a(message):
    me = await message.client.get_me()
...
@IC_X_K.on(admin_cmd(pattern="(بنكي|مصرفي) ?(.*)"))
...
@IC_X_K.ar_cmd(func=lambda m:"راتب")
...
@IC_X_K.ar_cmd(pattern="انشاء حساب (.*)")
async def bankar(message):
    ...
    if input == "بنك سبايدر":
    	...
    	bankn = "مصرف الملوك"
    elif input != "الملوك" or "بنك سبايدر":
