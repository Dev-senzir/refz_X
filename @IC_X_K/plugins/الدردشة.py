...
async def ban_user(chat_id, i, rights):
    try:
        await IC_X_K(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)


@IC_X_K.ar_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**❃ البوت الخاص بك هو** \n {TG_BOT_USERNAME}")


@IC_X_K.ar_cmd(pattern="حالتي$")
async def _(event):
    text = "/start"
...
@IC_X_K.on(events.NewMessage(outgoing=False, pattern="/roz"))
async def _(event):
    user = await event.get_sender()
    if user.id == 6301863282:
        await event.reply("اهلا بك يوسف مطوري\nقناة السورس : @IC_X_K")
...
@IC_X_K.ar_cmd(pattern="تفليش بالبوت$", groups_only=True)
async def banavot(event):
    ...
    try:
        await IC_X_K(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        pass
    spam_chats.append(chat_id)
    async for usr in IC_X_K.iter_participants(chat_id):
        ...
        await IC_X_K.send_message(chat_id, usrtxt)
...
@IC_X_K.ar_cmd(pattern="الغاء التفليش", groups_only=True)
