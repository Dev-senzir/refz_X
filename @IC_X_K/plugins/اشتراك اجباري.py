...
async def participant_check(channel, user_id):
    try:
        await IC_X_K(GetParticipantRequest(channel, int(user_id)))
        return True
...
async def fsub(event):
    ...
    if not str(channel).startswith("@"):
        ...
        if not await participant_check(channel, IC_X_K.uid):
            return await event.reply(
...
IC_X_K.ar_cmd(pattern="تعطيل الاجباري")
