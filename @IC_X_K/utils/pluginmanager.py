...
from IC_X_K import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"IC_X_K/plugins/{shortname}.py")
        checkplugins(path)
        name = f"IC_X_K.plugins.{shortname}"
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"IC_X_K.plugins.{shortname}"] = mod
        LOGS.info(f"تم تحميل ملف {shortname}")
...
def remove_plugin(shortname):
    ...
    try:
        name = f"IC_X_K.plugins.{shortname}"
        for i in reversed(range(len(IC_X_K._event_builders))):
            ev, cb = IC_X_K._event_builders[i]
            if cb.__module__ == name:
                del IC_X_K._event_builders[i]
